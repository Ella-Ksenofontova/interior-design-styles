 ...
 * @param {string} props.iconSrc - Source of decorative icon.
 * @returns {React.JSX.Element} The rendered InfoSourcesList component.
 */

export default function SourcesList({title, explanation = "", sources, iconSrc}) {
  return (
    <div className={styles["sources-list"]}>
      <img src={iconSrc} alt="" aria-hidden className={styles["list-icon"]} />
      <h2 tabIndex={0}>{title}</h2>
      {explanation ?
        <span className={styles.explanation} tabIndex={0}>{explanation}</span>
        : ""}
      <ul className = {styles["sources"]}>
        {sources.map((item, index) => <li key={`source-${index + 1}`}><a href={item.url}>{item.description}</a></li>
        )}
      </ul>
    </div>
  );
